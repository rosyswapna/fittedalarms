	...
	private $error = array(); 
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
    		...
    		$video = $this->model_catalog_video->getProductVideo($this->request->get['product_id']);

    		if(isset($this->request->post['video']) and trim($this->request->post['video']) != ""){

    			$video_description = array();
    			foreach ($this->request->post['product_description'] as $language_id => $value) {
    				$video_description[$language_id] = array(
    												'name'			=> $value['name'],
				...
				$vdata = array(
						'video_url'			=> $this->request->post['video'],
						'product_id'		=> $this->request->get['product_id'],
						'video_description'	=> $video_description
					);

				if($video){
					$video = $this->model_catalog_video->editVideo($video['video_id'],$vdata);
				}else{
					$video = $this->model_catalog_video->addVideo($vdata);
				}
			}else{
				if($video){
					$this->model_catalog_video->deleteVideo($video['video_id']);
				}
			}
			//-----------------------------------------------------------------------------------------------
