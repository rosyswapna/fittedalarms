...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
		...
		if (!empty($this->request->post['quantity'])) {
			...
			if (isset($this->request->post['addonquantity'])) {
				foreach ($this->request->post['addonquantity'] as $keyproduct => $addonarr) {
					$this->cart->updateaddon($keyproduct, $addonarr);
				}	
			}
		...
		if (isset($this->request->get['addonremove'])) {
			 //$this->request->get['addonremove'];
			//exit();
			$this->cart->addonremove($this->request->get['addonremove'],$this->request->get['parent_id']);
			
			unset($this->session->data['vouchers'][$this->request->get['addonremove']]);
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($productsfts as $productsft) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					...
					$price_raw=$this->tax->calculate($productsft['price'], $productsft['tax_class_id'], $this->config->get('config_tax'));
					$price_linetotal=$this->currency->format($productsft['quantity']*$productsft['price']);
				} else {
					$price = false;
				}
				...
				'product_id' =>$productsft['product_id'],
					...
					'price'    => $price,
					'price_linetotal'=>$price_linetotal,
					'total'    => $total,
...
'emptycart'   => $this->url->link('checkout/cart', 'emptycart=' . 1),
	...
	public function add_service()
	{
		...
		if(isset($this->request->post['service']) and $this->request->post['service'] > 0){

			$service_info = $this->model_checkout_services->getService($this->request->post['service']);

			if($service_info){
				$this->session->data['service_id'] = $service_info['service_id'];
				
				$this->session->data['service_charge'] = $service_info['service_charge'];

				$total = $this->cart->getSubTotal();

				$json['service_charge'] = $service_info['service_charge'];
			...
			$json['total'] = $this->currency->format($this->cart->getSubTotal());
		}
	...
	public function add_service_option()
	{
		...
		if(isset($this->request->post['service_option'])){

			$this->session->data['service_option'] = $this->request->post['service_option'];

			$total = $this->cart->getSubTotal();

			$json['service_option'] = $this->session->data['service_option'];
	...
	public function add_alarm_servicing() {
		$this->language->load('checkout/cart');
		...
		if($this->cart->checkAlarmServicing()){
			...
			if ($product_info) {
				...
				$this->session->data['alarm_servicing'] = $product_info['product_id'];
	...
	public function add_addon() {
		...
		if ($product_info) {			
			...
			if (!$json) {
				$this->cart->add_addon($this->request->post['addon_id'],$this->request->post['product_id'], $quantity, $option);
