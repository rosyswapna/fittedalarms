...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'price_old' 	   => $query->row['price'],
				'price_discount'    => ($query->row['discount'] ? true: false),
				...
				'more_info'    => $query->row['more_info'],
			
				'viewed'           => $query->row['viewed'],
				'product_type'	   => $query->row['product_type'],
				'parent_product_id'=> $query->row['parent_product_id']
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		...
		//---------------------------swapna-------------------
		elseif(!empty($data['feature_ids'])){
			$ids = explode(',', $data['feature_ids']);
			if(count($ids) == 1){
				$sql .= " AND p.product_id IN ( SELECT product_id
                    FROM " . DB_PREFIX . "product_features
                    WHERE feature_id = ".$data['feature_ids'].")";
			}elseif(count($ids) >1 ){
				$sql .= " AND p.product_id IN ( SELECT product_id
                    FROM " . DB_PREFIX . "product_features
                    WHERE feature_id IN(".$data['feature_ids'].")
                    GROUP BY product_id
			...
			//$sql .= " AND pft.feature_id IN(".$data['feature_ids'].")";
		}
		//---------------------------swapna-------------------
		...
		//---------------------------swapna-------------------
		else if(!empty($data['manufacturer_ids'])) {
			$sql .= " AND p.manufacturer_id IN(" . $data['manufacturer_ids'] . ")";
		}

		if (!empty($data['price_ranges'])) {
			$price_range_condition = "";
			foreach($data['price_ranges'] as $price_range_detail){
				if($price_range_condition){
					$price_range_condition .= " OR";
				}
				if(array_key_exists('min',$price_range_detail) and array_key_exists('max',$price_range_detail)){
					$price_range_condition .= " (p.price BETWEEN ".$price_range_detail['min']." AND ".$price_range_detail['max'].")";
				}
				elseif(array_key_exists('min',$price_range_detail)){
					$price_range_condition .= " (p.price >= ".$price_range_detail['min'].")";
				}
				elseif(array_key_exists('max',$price_range_detail)){
					$price_range_condition .= " (p.price <= ".$price_range_detail['max'].")";
				}
				
			}

			if($price_range_condition){
				$sql .= " AND (".$price_range_condition.")";
			}
	...
	public function getBestSellerProducts($limit='',$data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		if (!$product_data) { 
			...
			$sql = "SELECT product_id FROM " . DB_PREFIX . "best_seller ORDER BY sort_order ASC";
	...
	//get total count of best seller products
	public function getTotalBestSellerProducts($limit = '') {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	//--------------customised function for filter products with filters by swapna-----------------start here
	public function getTotalProductsWithoutCatagory($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		if (!empty($data['feature_ids'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "product_features pf ON (p.product_id = pf.product_id)";
		}
		...
		if (!empty($data['manufacture_ids'])) {
			$sql .= " AND p.manufacturer_id IN(" .$data['manufacture_ids'].")";
		}

		if (!empty($data['feature_ids'])) {
			$sql .= " AND pf.feature_id IN(" .$data['feature_ids'].")";
		}
		echo $sql;exit();
	...
	//--------------customised function for filter products with filters by swapna-----------------ends here
