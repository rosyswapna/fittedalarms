...
class ModelCatalogProduct extends Model {
		...
		//video updation----------------------
		if (isset($data['video']) and trim($data['video']) != "") {
			$this->db->query("INSERT INTO " . DB_PREFIX . "video SET video_url = '" . $this->db->escape(html_entity_decode($data['video'], ENT_QUOTES, 'UTF-8')) . "', product_id = '" . $this->db->escape($product_id) . "', date_added = NOW()");
	    	...
	    	$video = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "video v WHERE v.product_id = '" . (int)$product_id ."'");
	    	
	    	if($video->num_rows > 0){
	    		$this->db->query("DELETE FROM " . DB_PREFIX . "video WHERE video_id = '" . (int)$video['video_id']. "'");
      			$this->db->query("DELETE FROM " . DB_PREFIX . "video_description WHERE video_id = '" . (int)$video['video_id'] . "'");
	    	}
	...
	public function getProducts($data = array()) {
		...
		//swapna---------------------------------------------------
		if (isset($data['product_type']) and $data['product_type'] > 0) {
	...
	public function getProductDiscounts($product_id) {
		...
		public function getProductPackagesdet($product_id) {
	...
	public function getBestSellerLayoutProducts() {
		
		$product_data = array();
		if (!$product_data) { 
			$product_data = array();
			$sql = "SELECT product_id,sort_order FROM " . DB_PREFIX . "best_seller ORDER BY sort_order ASC";			
			...
			foreach ($query->rows as $result) { 
				...
				$product_data[$result['product_id']] = $this->getProduct($result['product_id']);
				$product_data[$result['product_id']]['bestseller_order'] = $result['sort_order'];
	...
	public function updateBestSeller($data) {
		
		if ($data) { 

			$this->db->query("DELETE FROM " . DB_PREFIX . "best_seller");

			foreach ($data as $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "best_seller SET product_id = '" . (int)$this->db->escape($value['product_id'])."', sort_order = '" . (int)$this->db->escape($value['sort_order']) . "'");
			}
