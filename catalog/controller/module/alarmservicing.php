<?php
class ControllerModuleAlarmservicing extends Controller {
	private $error_alarmservicing = array(); 
    public function index() {
        $this->language->load('module/alarmservicing'); //Optional. This calls for your language file
	...
	$this->data['cus_call_back'] = $this->language->get('cus_call_back');
      ...
      //  $this->data['text_content']  = $this->language->get('text_content');
		...
		$services = $this->model_checkout_services->getServices(3);
		foreach ($services as $service) {

		$features = $this->model_checkout_services->getServiceFeatures($service['product_id']);
		
			$this->data['services'][] = array(
				'product_id' => $service['product_id'],
				'name'        => $service['name'],
				'price_text'  => $this->currency->format($service['price']),
 ...
 //End of category
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/alarmservicing.tpl')) { //if file exists in your current template folder
            $this->template = $this->config->get('config_template') . '/template/module/alarmservicing.tpl'; //get it
        } else {
            $this->template = 'default/template/module/alarmservicing.tpl'; //or get the file from the default folder
        }
