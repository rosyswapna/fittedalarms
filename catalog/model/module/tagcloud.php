    ...
    private function generateTagCloud($data) 
    {
        ...
        if ($resize == true) {
            ...
            foreach ($tags as $record) {
                ...
                $cloud[] = '<a href="' . (HTTP_SERVER . 'index.php?route=product/search&filter_tag=' . $record['tag']) . '&tag=' . $record['tag'] . '" style="text-decoration: none;font-size: ' . $size . 'px" title="' . $record['qty'] . ' ' .$this->language->get('text_href_title') . ' ' . $record['tag'] . '">' . $record['tag'] . '</a> ';
            }
        } else {
            foreach ($tags as $record) {
                $cloud[] = '<a href="' . (HTTP_SERVER . 'index.php?route=product/search&filter_tag=' . $record['tag']) . '&tag=' . $record['tag'] . '" title="' . $record['qty'] . ' products tagged with ' . $record['tag'] . '">' . $record['tag'] . '</a> ';
            }
        }
