...
<script type="text/javascript">
{literal}
function set_return_specific(document_id, document_name) 
{
    if (opener.document.getElementById('document_attachment_'+document_id))
    {
        alert("{/literal}{$EMMOD.LBL_DOCUMENT_IS_ALREADY_ADDED}{literal}");
    }
    else
    {
        opener.addDocumentIntoEmail(document_id, document_name);
    }
}
...
{ldelim}
	...
	{ldelim}
        ...
        email_exists = window.opener.document.getElementById('emailadress_{$PID}_'+entity_id+'_'+email_id);
        if (!email_exists)
        {ldelim}
            {if $TYPE neq "1"}
                var el_name = '{$PID}_'+entity_id+'_'+email_id;
            {else}
                var el_name = entity_id+'_'+entity_id+'_'+email_id;
            {/if}
            
            /* var emaillist_content = window.opener.document.getElementById('div_emaillist{if $TYPE neq "1"}_{$PID}{/if}').innerHTML;*/
            
            var selectbox = '<select name="ToEmails[]" id="to_email_' + el_name + '" style="font-size:10px;"><option value="normal_' + el_name + '">{$EMOD.LBL_TO}</option><option value="cc_' + el_name + '">{$EMOD.LBL_CC}</option><option value="bcc_' + el_name + '">{$EMOD.LBL_BCC}</option></select>';
            
            var clear_btn = '<span ><img src="themes/images/clear_field.gif" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="clearEmailFromTable(\'{if $TYPE neq "1"}{$PID}{else}'+entity_id+'{/if}\',\''+entity_id+'\',\''+email_id+'\',this); return false;" align="absmiddle" style="cursor:hand;cursor:pointer"></span>';
            
            border_style = '1px solid #C0C0C0';
            
            
            var new_tr = window.opener.document.getElementById('table_emaillist{if $TYPE neq "1"}_{$PID}{/if}').insertRow(0);
            new_tr.id = 'emailadress_{$PID}_'+entity_id+'_'+email_id;
            ...
            td2.innerHTML = parentname + " <i>&lt;" + emailadd + "&gt;</i>";
            //td2.style = "font-size:10px;line-height:12px;" + border_top + border_bottom;
            ...
            /*
            my_div = window.opener.document.getElementById('div_emaillist{if $TYPE neq "1"}_{$PID}{/if}');
            
            if (emaillist_content == "&nbsp;")
            {ldelim}
                my_div.innerHTLM = ""; 
            {rdelim}
            ...
            newDiv.id = 'emaildiv_' + el_name;
            newDiv.setAttribute('title',parentname + ' - ' + emailadd);
            ...
            newDiv.style.cssText = border_top + 'border-right:1px solid #C0C0C0;height:27px;overflow:hidden;';

            newDiv.innerHTML = '<table width="100%"><tr><td width="10px">' + selectbox + '</td><td style="font-size:10px;line-height:12px;">'+ parentname + '</td><td width="10px">' + clear_btn + '</td></tr></table>'
