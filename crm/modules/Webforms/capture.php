...
class Webform_Capture {
	...
	function captureNow($request) {
		...
		try {
			...
			$this->sendResponse($returnURL, 'ok',false,$this->webform_id);
			return;

		} catch (Exception $e) {
			$this->sendResponse($returnURL, false, $e->getMessage(),$this->webform_id);
			return;
		}
	}

	protected function sendResponse($url, $success=false, $failure=false,$webform = 'Request') {
		if (empty($url)) {
			if ($success) $response = Zend_Json::encode(array('success' => true, 'result' => $success,'from'=>$webform));
			else $response = Zend_Json::encode(array('success' => false, 'error' => array('message' => $failure), 'from'=>$webform));
			...
			header(sprintf("Location: http://%s?%s=%s&%s=%s", $url, ($success? 'success' : 'error'), ($success? $success: $failure),'from',$webform));
		}
	}
}
